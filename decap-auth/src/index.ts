...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/" || url.pathname === AUTH_PATH) {
      ...
      githubUrl.searchParams.set("state", DEFAULT_STATE);
    ...
    if (url.pathname === CALLBACK_PATH) {
      ...
      if (error) {
        return new Response(html(`authorization:github:error:${JSON.stringify({ message: error })}`, url.origin), {
          headers: { "content-type": "text/html; charset=utf-8" },
        });
      }

      if (!code) {
        return new Response(html(`authorization:github:error:${JSON.stringify({ message: "missing_code" })}`, url.origin), {
          status: 400,
          headers: { "content-type": "text/html; charset=utf-8" },
        });
      }
      ...
      try {
        ...
        return new Response(
          html(`authorization:github:success:${JSON.stringify({ token })}`, url.origin),
          {
          headers: { "content-type": "text/html; charset=utf-8" },
          },
        );
        ...
        return new Response(
          html(`authorization:github:error:${JSON.stringify({ message })}`, url.origin),
          {
          status: 500,
          headers: { "content-type": "text/html; charset=utf-8" },
          },
